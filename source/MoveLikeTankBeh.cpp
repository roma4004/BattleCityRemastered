...
{
	if (const float speed = _speed * deltaTime;
		_selfParent->GetRightSide() + speed < static_cast<float>(_windowSize.x))
	{
		if (const auto pawns = IsCanMove(deltaTime); pawns.empty())
		{
			_selfParent->MoveX(std::floor(speed));
		...
		{
			auto getSideDiff = [thisRightSide = _selfParent->GetRightSide()](
					const std::shared_ptr<BaseObj>& pawn) -> float
			{
		...
		_selfParent->GetBottomSide() + speed < static_cast<float>(_windowSize.y))
	{
		...
		else
		{
			const auto getSideDiff = [thisBottomSide =
						_selfParent->GetBottomSide()](const std::shared_ptr<BaseObj>& pawn) -> float
			{
