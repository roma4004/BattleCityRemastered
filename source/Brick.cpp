	...
	: BaseObj(pos, width, height, color, speed, health, env)
{
	...
	_env->events.AddListenerToEvent("MarkDestroy", eventName,
									[env = _env, self = dynamic_cast<Pawn*>(this)]() { self->MarkDestroy(env); });

	_env->events.AddListenerToEvent("Draw", eventName,
...
Brick::~Brick()
{
	...
	if (const auto it = std::ranges::find(_env->allPawns, dynamic_cast<Pawn*>(this)); it != _env->allPawns.cend())
	{
