...
std::unique_ptr<IGame> ConfigSuccess::CreateGame()
{
	...
	auto menu = std::make_unique<Menu>(_renderer, _fpsFont, _logoTexture, statistics, _windowSize,
	                                   events);
	...
	return std::make_unique<GameSuccess>(
			_windowSize, events, statistics, std::move(menu), textureManager, _isVsyncOn, stateManager);
}
