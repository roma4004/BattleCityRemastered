...
ConfigSuccess::ConfigSuccess(const UPoint windowSize, boost::property_tree::ptree& pTreeIni,
							 ...
							 const std::shared_ptr<SDL_Texture>& atlas,
...
std::unique_ptr<IGame> ConfigSuccess::CreateGame(const GameMode gameMode)
{
	...
	auto renderManager = std::make_unique<RenderManager>(events, _renderer, _pTreeIni, buttonTexturesPS5,
	...
	return std::make_unique<GameSuccess>(_windowSize, _pTreeIni, events, menu, renderManager, rightSideBar, gameMode);
}
