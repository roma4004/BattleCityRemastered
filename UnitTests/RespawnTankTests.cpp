...
TEST_F(TankSpawnerTest, EnemyOneRespawn)
{
	...
	_respawnManager->SetSlotNeedRespawn(static_cast<int>(TankType::ENEMY1));
...
TEST_F(TankSpawnerTest, EnemyTwoRespawn)
{
	...
	_respawnManager->SetSlotNeedRespawn(static_cast<int>(TankType::ENEMY2));
...
TEST_F(TankSpawnerTest, EnemyThreeRespawn)
{
	...
	_respawnManager->SetSlotNeedRespawn(static_cast<int>(TankType::ENEMY3));
...
TEST_F(TankSpawnerTest, EnemyFourRespawn)
{
	...
	_respawnManager->SetSlotNeedRespawn(static_cast<int>(TankType::ENEMY4));
...
TEST_F(TankSpawnerTest, PlayerOneDiedRespawn)
{
	...
	_respawnManager->SetSlotNeedRespawn(static_cast<int>(TankType::PLAYER1));
...
TEST_F(TankSpawnerTest, PlayerTwoDiedRespawn)
{
	...
	_respawnManager->SetSlotNeedRespawn(static_cast<int>(TankType::PLAYER2));
...
{
	const int respawnCount = _respawnManager->GetEnemyRespawnCount();//TODO: move to separated test group
	_respawnManager->SetSlotNeedRespawn(static_cast<int>(TankType::ENEMY2));
	constexpr bool skipDelay = true;
	_tankSpawner->RespawnTanks(skipDelay);
	_allObjects.pop_back();

	EXPECT_GT(respawnCount, _respawnManager->GetEnemyRespawnCount());
}

TEST_F(TankSpawnerTest, PlayerOneDiedRespawnCount)
{
	const int respawnCount = _respawnManager->GetPlayerOneRespawnCount();
	_respawnManager->SetSlotNeedRespawn(static_cast<int>(TankType::PLAYER1));
	constexpr bool skipDelay = true;
	_tankSpawner->RespawnTanks(skipDelay);
	_allObjects.pop_back();

	EXPECT_GT(respawnCount, _respawnManager->GetPlayerOneRespawnCount());
}

TEST_F(TankSpawnerTest, PlayerTwoDiedRespawnCount)
{
	const int respawnCount = _respawnManager->GetPlayerTwoRespawnCount();
	_respawnManager->SetSlotNeedRespawn(static_cast<int>(TankType::PLAYER2));
	constexpr bool skipDelay = true;
	_tankSpawner->RespawnTanks(skipDelay);
	_allObjects.pop_back();

	EXPECT_GT(respawnCount, _respawnManager->GetPlayerTwoRespawnCount());
}

TEST_F(TankSpawnerTest, EnemyRunOutRespawnPoints)
{
	const int respawnCount = _respawnManager->GetEnemyRespawnCount();
	_respawnManager->SetSlotNeedRespawn(static_cast<int>(TankType::ENEMY2));
	...
	EXPECT_GT(respawnCount, _respawnManager->GetEnemyRespawnCount());
	EXPECT_EQ(0, _respawnManager->GetEnemyRespawnCount());
}
...
TEST_F(TankSpawnerTest, PlayerOneRunOutRespawnPoints)
{
	...
	EXPECT_GT(respawnCount, _respawnManager->GetPlayerOneRespawnCount());
	EXPECT_EQ(0, _respawnManager->GetPlayerOneRespawnCount());
}
...
TEST_F(TankSpawnerTest, PlayerTwoRunOutRespawnPoints)
{
	...
	EXPECT_GT(respawnCount, _respawnManager->GetPlayerTwoRespawnCount());
	EXPECT_EQ(0, _respawnManager->GetPlayerTwoRespawnCount());
}
