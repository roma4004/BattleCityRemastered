...
{
	if (auto obstacle = std::make_shared<TObstaclesType>(rect, _events, uuid, _gameMode);
...
{
	if (auto fortressWall = std::make_shared<FortressWall>(rect, _events, _allObjects, uuid, _gameMode);
